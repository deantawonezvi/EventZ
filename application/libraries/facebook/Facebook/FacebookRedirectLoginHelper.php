...
class FacebookRedirectLoginHelper
{
  ...
  //returns the url for authenticating a request
  public function getAuthUrl(){
    $state = $this->loadState();
    $params1 = array('client_id' => $this->appId);
    $params3 = array('secret' => $this->appSecret);

      $data = array('url' => 'https://graph.facebook.com/oauth/access_token?' . http_build_query($params1, null, '&')."&redirect_uri=".$this->redirectUrl."&".http_build_query($params3, null, '&'),
                    'state'=> $state);
      return $data;
  }
